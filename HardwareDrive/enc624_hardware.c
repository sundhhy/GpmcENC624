...
#define ENC624_MOSI_HighASM 
#define ENC624_MOSI_LowASM 

#define ENC624_SCK_HighASM 
#define ENC624_SCK_LowASM 

#define ENC624_MISO       (PORTB & (1<<3))

#define ENC624_CS_HighASM 
#define ENC624_CS_LowASM 
...
/****                                                                      ****/
/******************************************************************************/
#define    enc624j600_spi_Tx(cmd) enc624j600_spi_TxRx(cmd)
	
UI08_t	enc624j600_spi_Rx(void) 
{   
	UI08_t data;
	
	data=enc624j600_spi_TxRx( 0  );
...
{
    enc624j600SpiCommand(WRXRDPT , (UI08_t*)&ptr, sizeof(UI16_t));
...
UI16_t enc624j600_readRegister(UI08_t addr)
{
    ...
    UI16_t dat=enc624j600_spi_TxRx((addr & 0b11111)); // 000a aaaa
    //UI16_t dat = enc624j600_spi_Rx();
    dat |= (enc624j600_spi_TxRx(0) << 8);
    ENC624_CS_HighASM;
...
void enc624j600_writeRegister(UI16_t addr, UI16_t v)
{
    //
    ENC624_CS_LowASM;
    enc624j600_spi_TxRx(0b01000000 | (addr & 0b11111)); // 000a aaaa
    enc624j600_spi_TxRx(v & 0xFF);
    enc624j600_spi_TxRx(v >> 8);
    ENC624_CS_HighASM;
}
...
void enc624j600SpiWriteRegister8(UI08_t addr, UI08_t value)
{
    ...
    ENC624_CS_LowASM;
    enc624j600_spi_TxRx((1<<6) | (addr & 0b11111)); // 010a aaaa
    enc624j600_spi_TxRx(value);
    ENC624_CS_HighASM;
}
...
void enc624j600SpiBSetRegister8(UI08_t addr, UI08_t value)
{
    ...
    ENC624_CS_LowASM;
    enc624j600_spi_TxRx((1<<7) | (addr & 0b11111)); // 100a aaaa
    enc624j600_spi_TxRx(value);
    ENC624_CS_HighASM;
}
...
void enc624j600_setBank(UI08_t bank)
{
    ...
    {
        ENC624_CS_LowASM;
        enc624j600_spi_TxRx(0b11000000 | (bank<<1));
        ENC624_CS_HighASM;
